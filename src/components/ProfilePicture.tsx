...
export default function ProfilePicture({picture, width, height}:{picture:string, width: string, height: string}) {
  if(!picture){
    return <img src="/public/img/image.png" className={`${width} ${height} rounded-full object-cover border`} />;
  }
  ...
  const src = isFromS3 ? (isSuccess ? data?.url : "/public/img/image.png") : picture;
